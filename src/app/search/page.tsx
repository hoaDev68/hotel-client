                ...
                {rooms.map((roomData, index) => (
                  <tr key={roomData.room._id} className="border-b hover:bg-gray-50">
                    ...
                    <td
                      ...
                      rowSpan={index === 0 ? rooms.length : undefined}
                      style={{ display: index === 0 ? 'table-cell' : 'none' }}
                    >
                      {(() => {
                        const minCapacity = parseInt(searchParams.get('minCapacity') || '0')
                        const totalGuests = selectedRooms.reduce((sum, selectedRoom) => {
                          const roomData = rooms.find(r => r.room._id === selectedRoom.roomId)
                          return sum + (roomData?.room.capacity || 0) * selectedRoom.quantity
                        }, 0)

                        if (selectedRooms.length === 0) {
                          return <p className="text-sm text-gray-500 italic">No rooms selected</p>
                        }

                        if (totalGuests < minCapacity) {
                          return (
                            <div className="text-red-600 text-sm font-medium">
                              Vui lòng chọn thêm phòng để đảm bảo đủ số lượng người ({totalGuests}/{minCapacity})
                            </div>
                          )
                        }
                          ...
                          <div className="space-y-4">
                            <div className="space-y-2 text-sm">
                              {selectedRooms.map((selectedRoom) => {
                                ...
                                return (
                                  <div key={selectedRoom.roomId} className="flex justify-between items-center">
